# Skill: Generate Playwright HTML Report

Generate a modern, self-contained HTML report from Playwright JSON test output.

---

## What this skill does

1. Copies `generate-report.js` into the project root (full source below).
2. Adds an npm script shortcut to `package.json`.
3. Updates `COMMANDS.md` with the run + report commands.

Invoke when the user asks to "generate a report", "create an HTML report", or "set up reporting" for a Playwright project.

---

## Usage (after setup)

### Step 1 — Run tests, export JSON

```bash
npx playwright test <spec-file> --reporter=json 2>/dev/null > /tmp/pw-results.json
```

### Step 2 — Generate the HTML report

```bash
node generate-report.js --input=/tmp/pw-results.json --feature=<feature-name>
```

**Output:** `reports/<feature-name>_DD_MM_YYYY_HH_MM_SS.html`
...
| `--feature` | No       | Feature name prefix for the output file (defaults to suite name)  |
| `--output`  | No       | Override full output path, e.g. `--output=reports/my-report.html` |

---

## Report features

- **Dark theme** — Tailwind CSS via CDN, slate/dark background
- **KPI cards** — Total / Passed / Failed / Flaky / Skipped / Duration
- **Donut chart** — SVG pass-rate chart (no external libs)
- **Environment panel** — Browser, platform, base URL, execution time
- **Suite accordions** — Collapsible per-file/per-suite sections
- **Per-test detail** — Steps, error messages, stack traces (collapsed by default, click to expand)
- **Attachments** — Screenshots and videos inline (fails gracefully if files missing)
- **Flakiness detection** — Tests that failed then passed on retry are marked ⚡ Flaky
- **Filter bar** — Filter by All / Passed / Failed / Flaky / Skipped
- **Search box** — Live search by test title
- **Output naming** — `reports/<feature>_DD_MM_YYYY_HH_MM_SS.html`

---

## generate-report.js — full source

Copy this file verbatim to the project root as `generate-report.js`.
  ...
  function walkSuites(suiteList, parentFile) {
    for (const suite of (suiteList ?? [])) {
      ...
      if (suite.specs?.length) {
        ...
        if (tests.length) suites.push({ file, title: suite.title, tests });
      }
...
function buildTest(spec, test, suiteTitle) {
  ...
  if (attempts === 0) {
    ...
    if (hasFailure && lastPassed)             status = 'flaky';
    else if (results.every(r => r.status === 'passed')) status = 'passed';
    else                                      status = 'failed';
  } else {
...
function flattenSteps(steps, depth = 0) {
  ...
  for (const step of (steps ?? [])) {
    result.push({ title: step.title ?? '', duration: step.duration ?? 0, error: step.error ?? null, depth });
    if (step.steps?.length) result.push(...flattenSteps(step.steps, depth + 1));
  }
  return result;
}
...
function deriveFeatureName(data) {
  ...
  return path.basename(raw, path.extname(raw))
    .replace(/[^a-zA-Z0-9_-]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '').toLowerCase();
}
...
function renderHtml({ summary, meta, suites }, { featureName, generatedAt }) {
  ...
  const statusIcon  = s => ({ passed: '✓', failed: '✕', flaky: '⚡', skipped: '–' }[s] ?? '?');
  const statusBadge = s => ({
          ...
          ${videos.map(a => `
            <div>
              ...
              <video src="${escHtml(a.path)}" controls
                     class="rounded-lg border border-slate-700 max-h-64 w-full bg-slate-900"></video>
            </div>`).join('')}
        </div>` : '';
...
- [ ] Copy `generate-report.js` to the project root
- [ ] Create a `reports/` folder (or let the script create it automatically)
- [ ] Add to `package.json` scripts:
  ```json
  "report:generate": "node generate-report.js",
  "report:<feature>": "npx playwright test <spec> --reporter=json 2>/dev/null > /tmp/pw-results.json && node generate-report.js --input=/tmp/pw-results.json --feature=<feature>"
  ```
- [ ] Add `reports/` to `.gitignore` if you don't want to commit generated reports

---

## Key implementation notes

- **`process.stdin.isTTY === false`** (strict check) — `isTTY` can be `undefined` in some Node/shell contexts, not just `true`/`false`. Falsy check (`!isTTY`) would accidentally read stdin when no pipe is connected.
- **Flakiness** — a test with `attempts > 1` where earlier results failed but the last passed is marked `flaky`, not `passed`.
- **AntD / framework-specific** — this script is framework-agnostic; it reads standard Playwright JSON. No changes needed for different UI frameworks.
- **Self-contained HTML** — Tailwind via CDN, inline SVG donut, no build step required.
- **Output path** — always `reports/<feature>_DD_MM_YYYY_HH_MM_SS.html` unless `--output` overrides it.
